...
var Player = {
	createNew: function() {
		...
		player.showchar = function() {
			rstr = '+==========================+\n';
			rstr += name + '\n';
			rstr += 'STR: ' + status_search('str') + ' DEX: ' + status_search('dex') + ' CON: ' + status_search('con') + '\n';
			rstr += 'POW: ' + status_search('pow') + ' APP: ' + status_search('app') + ' INT: ' + status_search('int') + '\n';
			rstr += 'SIZ: ' + status_search('siz') + ' EDU: ' + status_search('edu') + ' DB: ' + status_search('db') + '\n';
			rstr += '+--------------------------+\n';
			rstr += 'HP: ' + status_search('hp') + ' MP: ' + status_search('str')mp + ' SAN: ' + status_search('san') + '\n';
			rstr += 'STATUS: ' + status_search('status') + '\n';
			rstr += 'ITEM: ' + status_search('item') + '\n';
			rstr += '+==========================+\n';
			return rstr;
		}
		...
		player.set = function(string, value) {
			...
			if(pos =='-1'){
				...
				if(value.charAt(0).toString() == '+'){
					value = status_getVal(string)*1 + value.substr(1,value.length-1)*1;
		...
		player.delete = function() {
			var name = '';
			var db='', item='', status='';
			var skill_10 = '0000000000000008601110102221300110222210100000001101000000000000002222111101221111000000000000000';
			var skill_01 = '0000000000000005505505015000011000000005055555550010111111111111110500550055055000555500000000000';
			var other_skills = ['', '', '', '', '', '' ,'' ,'' ,'' ,''];
			var rstr='';
		}
		...
		player.status_search = function(string) {
			...
			else return string + ': ' + player.status_getVal(tempstr);
		}
		...
		player.status_getVal = function(string) {
			var tempstr;
...
function CharacterControll(trigger, str1, str2){
	...
	if(trigger == 'new' || trigger == '建立'){
		...
		for(i=0; i<5; i++) {
			if(players[i].status_search('name') == str1) return '已經有同名的角色了!';
		}
		
		for(i=0; i<5; i++) {
			if(players[i].status_search('name') == '') {
	...
	//角色設定(特定狀態查詢) 刪除 查看
	for(i=0; i<5; i++) {
		if(trigger == players[i].status_search('name')){
			if(str1 == 'show' || str1 == undefined || str1 == '' || str1 == '狀態' || str1 == '屬性') {
				return players[i].showchar();
			}
	...
	if(trigger == 'list' || trigger == '清單') {
		...
		for(i=0; i<5; i++){
			tempstr += i + '. ' + players[i].status_search('name') + '\n';
		}
		return tempstr;
	}
	return '查無此角色';
